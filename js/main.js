(() => {
  ...
  // Projetos ocultos no painel (published: false) não aparecem no site
  const PROJECTS = (window.PROJECTS || []).filter(p => p.published !== false);
  ...
  /* ---------- abertura: slides gerados a partir dos projetos marcados como destaque ---------- */
  const heroSlides = $(".js-hero-slides");
  if (heroSlides) {
    let featured = PROJECTS.filter(p => p.featured && p.cover);
    if (!featured.length) featured = PROJECTS.filter(p => p.cover).slice(0, 4);
    heroSlides.innerHTML = featured.map((p, i) => `
      <div class="hero__slide${i === 0 ? " is-active" : ""}" data-title="${esc(p.title)}" data-sub="${esc(p.place || p.location || p.category)}" data-href="projeto.html?p=${esc(p.slug)}">
        <img src="${esc(p.cover)}" alt="${esc(p.title)}" style="object-position:${esc(p.heroPos || "50% 50%")}"${i === 0 ? "" : ' loading="lazy"'}>
      </div>`).join("");
    const f = featured[0];
    if (f) {
      $(".js-hero-title").textContent = f.title;
      $(".js-hero-sub").textContent = f.place || f.location || f.category || "";
      $(".hero__caption a").setAttribute("href", `projeto.html?p=${f.slug}`);
    }
  }
  ...
  if (filters) {
    // filtros gerados a partir das categorias existentes
    const cats = [...new Set(PROJECTS.map(p => p.category).filter(Boolean))];
    filters.innerHTML = `<li><button class="is-active" data-filter="*">Todos</button></li>` +
      (cats.length > 1 ? cats.map(c => `<li><button data-filter="${esc(c)}">${esc(c)}</button></li>`).join("") : "");
  }
  if (grid) {
    grid.innerHTML = PROJECTS.map(p => `
      <a class="pcard" href="${projectHref(p.slug)}" data-cat="${esc(p.category)}">
        <div class="pcard__media reveal-img">
          <img src="${esc(p.cover)}" alt="${esc(p.title)}" loading="lazy">
          ${p.hover ? `<img class="pcard__hover" src="${esc(p.hover)}" alt="" loading="lazy">` : ""}
        </div>
        <div class="pcard__info">
          <h3 class="pcard__title">${esc(p.title)}</h3>
          <span class="pcard__meta"><span>${esc(p.category)}</span>${p.place ? `<i>|</i><span>${esc(p.place)}</span>` : ""}${p.year ? `<i>|</i><span>${esc(p.year)}</span>` : ""}</span>
        </div>
      </a>`).join("");
  ...
  function renderProject(slug, main) {
    ...
    const text = (p.text || []).filter(Boolean);
    document.title = `${p.title} — Tey Romeiro Arquitetura`;
    const md = $('meta[name="description"]'); md && (md.content = p.lead || p.title);
    lbItems = gallery;

    const handle = s => esc(s).replace(/@([\w.]+)/g, (m, h) => `<a href="https://www.instagram.com/${h}/" target="_blank" rel="noopener">${m}</a>`);
    const facts = [
      p.place && ["Localização", p.place],
      p.category && ["Tipologia", p.category],
      p.year && ["Ano", p.year],
      p.area && ["Área", p.area],
      p.status && ["Status", p.status],
      p.tags && p.tags.length && ["Escopo", p.tags.join(" · ")],
      ...(p.credits || []).filter(r => r && r[0] && r[1])
    ].filter(Boolean);
    ...
    main.innerHTML = `
      <div class="container">
        <div class="p-head">
          <div>
            <span class="label label--dash">${esc(p.category)}${p.place ? " · " + esc(p.place) : ""}</span>
            <h1 class="title">${esc(p.title)}</h1>
          </div>
          <a href="${homeHref("projetos")}" class="link-line">← Todos os projetos</a>
        </div>
        ${p.cover ? `<div class="p-cover"><img src="${esc(p.cover)}" alt="${esc(p.title)}" style="object-position:${esc(p.heroPos || "center")}"></div>` : ""}
      </div>
      ...
      <section class="section">
        <div class="container p-intro">
          <aside class="p-facts reveal">
            ...
            <dl>${facts.map(([k, v]) => `<div><dt>${esc(k)}</dt><dd>${handle(String(v))}</dd></div>`).join("")}</dl>
          </aside>
          <div class="p-text">
            ${p.lead ? `<p class="statement reveal">${esc(p.lead)}</p>` : ""}
            ${text.map((t, j) => `<p class="${j === 0 ? "body-lg" : ""} reveal">${esc(t)}</p>`).join("")}
          </div>
        </div>
      </section>
      ...
      <section style="padding-bottom:clamp(56px,7vw,100px)">
        <div class="container gallery">
          ${gallery.map((g, j) => `<figure class="g-item${g.layout ? " g-item--" + esc(g.layout) : ""} reveal-img" data-i="${j}"><img src="${esc(g.src)}" alt="${esc(g.alt || p.title)}" loading="lazy"></figure>`).join("")}
        </div>
      </section>
      ...
      <div class="container">
        <a class="p-next" href="${projectHref(next.slug)}">
          <div>
            ...
            <p class="title">${esc(next.title)}</p>
            <span class="link-line">Ver projeto →</span>
          </div>
          <div class="p-next__media">${next.cover ? `<img src="${esc(next.cover)}" alt="" loading="lazy" style="object-position:${esc(next.heroPos || "center")}">` : ""}</div>
        </a>
      </div>` : ""}`;
